...
inline void push_literal(StrBuff *buff, char c) {
  ...
  while ((n = fread(buff, 1, chunk_size, src)) != 0) {
    ...
    while (i < n) {
      ...
      case STRING_LITERAL:
        if (flags == 2) {
          if (c == 't') {
            push_literal(char_stack, (char) 0x09);
          } else if (c == 'n') {
            push_literal(char_stack, (char) 0x0A);
          } else if (c == 'r') {
            push_literal(char_stack, (char) 0x0D);
          } else if (c == '"') {
            push_literal(char_stack, '"');
          } else if (c == '\\') {
            push_literal(char_stack, '\\');
          }
          flags >>= 1;
